// src/pages/memberHistory.ts
import { collection, getDocs, query, where, orderBy } from "firebase/firestore";
import { db } from "../firebaseConfig";
import type { User } from "firebase/auth";
import { loadMemberLoanDetails } from "./memberLoanDetails.ts";
...
export async function loadMemberLoanHistory(user: User) {
  ...
  if (activeSnap.empty) {
    ...
    const loanDoc = activeSnap.docs[0];
    const loan = loanDoc.data();

    html += `
      <div class="card clickable" data-loan-id="${loanDoc.id}">
